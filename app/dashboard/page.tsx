...
export default function Dashboard() {
  ...
  const lbl: any = { fontSize: 14, fontWeight: 500, color: '#5F5E5A', display: 'block', marginBottom: 6 }
  const inp: any = { width: '100%', padding: '10px 12px', border: '1px solid #E2E0D8', borderRadius: 8, fontSize: 16, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', background: '#FAFAF8' }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F7F4', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ background: '#0F172A', height: 54, padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <span style={{ color: '#fff', fontWeight: 500, fontSize: 18 }}>Apply<span style={{ color: '#1D9E75' }}>AI</span></span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {profile?.plan === 'free' && <button onClick={upgradeNow} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '6px 14px', borderRadius: 7, fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}><Crown size={12}/>Upgrade Pro</button>}
          {profile?.plan !== 'free' && <span style={{ fontSize: 14, color: '#1D9E75', display: 'flex', alignItems: 'center', gap: 4 }}><Crown size={12}/>{profile?.plan === 'africa' ? 'Global' : 'Pro'}</span>}
          <button onClick={signOut} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#64748B', display: 'flex', alignItems: 'center', gap: 4, fontSize: 15, fontFamily: 'inherit' }}><LogOut size={13}/>Sign out</button>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '24px 20px' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 10, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                <span style={{ fontSize: 15, fontWeight: 500 }}>Free plan — {freeRemaining} applications remaining this month</span>
                <span style={{ fontSize: 14, color: '#888780' }}>5 per month</span>
              </div>
            ...
            <button onClick={upgradeNow} style={{ fontSize: 14, fontWeight: 500, color: '#1D9E75', background: '#E1F5EE', border: 'none', padding: '5px 12px', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit' }}>Go Pro</button>
          </div>
        )}
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #E2E0D8', marginBottom: 24, gap: 4 }}>
          {[{ key: 'run', label: 'Apply now', icon: Zap }, { key: 'applications', label: `Applications (${applications.length})`, icon: Briefcase }, { key: 'account', label: 'Account', icon: Settings }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key as Tab)} style={{ padding: '10px 16px', fontSize: 15, fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', color: tab === t.key ? '#1D9E75' : '#888780', borderBottom: `2px solid ${tab === t.key ? '#1D9E75' : 'transparent'}`, marginBottom: -1, display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
          ...
          <div>
              ...
              <div style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 12, padding: 24, marginBottom: 16 }}>
                ...
                {error === 'limit' && (
                  <div style={{ background: '#FAEEDA', border: '1px solid #FAC775', borderRadius: 8, padding: '14px 16px', marginBottom: 16 }}>
                    <div style={{ fontWeight: 500, fontSize: 16, color: '#633806', marginBottom: 4 }}>Monthly limit reached</div>
                    <p style={{ fontSize: 15, color: '#854F0B', margin: '0 0 12px' }}>Upgrade to Pro for 100 applications/month.</p>
                    <button onClick={upgradeNow} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '9px 18px', borderRadius: 7, fontSize: 15, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Upgrade to Pro →</button>
                  </div>
                )}
                {error && error !== 'limit' && <div style={{ display: 'flex', gap: 8, background: '#fff5f5', border: '1px solid #F09595', borderRadius: 8, padding: '10px 12px', marginBottom: 16 }}><AlertCircle size={14} color="#A32D2D"/><span style={{ fontSize: 15, color: '#A32D2D' }}>{error}</span></div>}
                <button onClick={runAgent} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: 9, fontSize: 16, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'inherit' }}>
            ...
            {['profile','jobs','tailor'].includes(step) && (
              <div style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 12, padding: 32, textAlign: 'center' }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 500, margin: '0 0 8px' }}>
                  {step === 'profile' ? 'Analysing your CV...' : step === 'jobs' ? 'Finding matching jobs...' : 'Tailoring your CV...'}
                </h3>
                <p style={{ fontSize: 15, color: '#888780', margin: 0 }}>This usually takes under a minute</p>
              </div>
            )}
              ...
              <div>
                ...
                <div style={{ background: '#fff', border: '2px solid #1D9E75', borderRadius: 12, padding: 20, marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <div style={{ fontSize: 14, color: '#1D9E75', fontWeight: 500, marginBottom: 4 }}>BEST MATCH — {result.topJob.matchScore}%</div>
                      <h3 style={{ fontSize: 19, fontWeight: 500, margin: '0 0 4px' }}>{result.topJob.title}</h3>
                      <div style={{ fontSize: 16, color: '#5F5E5A' }}>{result.topJob.company} · {result.topJob.location} · {result.topJob.salary}</div>
                <a href={result.topJob.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 16, color: '#1D9E75', fontWeight: 500, textDecoration: 'underline' }}>View posting →</a>
                    </div>
                    <div style={{ background: '#E1F5EE', color: '#085041', fontWeight: 500, fontSize: 20, padding: '8px 14px', borderRadius: 8 }}>{result.topJob.matchScore}%</div>
                  </div>
                  {result.recruiterEmail && (
                    <div style={{ fontSize: 14, color: '#5F5E5A', marginBottom: 12 }}>
                  ...
                  {profile?.auto_apply_enabled ? (
                    <div>
                      <button onClick={submitViaEmail} disabled={submitting} style={{ background: submitting ? '#9FE1CB' : '#0F172A', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: submitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 7, fontFamily: 'inherit' }}>
                      ...
                      {submitResult && (
                        <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 8, background: submitResult.success ? '#E1F5EE' : '#fff5f5', border: `1px solid ${submitResult.success ? '#9FE1CB' : '#F09595'}` }}>
                          <span style={{ fontSize: 15, color: submitResult.success ? '#085041' : '#A32D2D' }}>{submitResult.message || submitResult.error}</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div style={{ background: '#FAEEDA', borderRadius: 8, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontSize: 14, color: '#854F0B', flex: 1 }}>Set up auto-apply to send this from your email automatically</span>
                      <button onClick={() => setTab('account')} style={{ fontSize: 13, color: '#854F0B', background: '#FAEEDA', border: '1px solid #FAC775', padding: '3px 10px', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit' }}>Set up →</button>
                    </div>
                  )}
                </div>
                  ...
                  <div key={s.key} style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 10, marginBottom: 10, overflow: 'hidden' }}>
                    ...
                    {expandedSection === s.key && (
                      <div style={{ padding: '0 16px 16px', borderTop: '1px solid #F1EFE8' }}>
                        {s.content ? (
                          <>
                            <pre style={{ fontSize: 15, lineHeight: 1.7, whiteSpace: 'pre-wrap', margin: '12px 0 8px', fontFamily: 'inherit' }}>{s.content}</pre>
                          ...
                          <div style={{ paddingTop: 10 }}>
                            {result.atsKeywords && Object.entries({ 'Must-have': result.atsKeywords.mustHave, 'Technical': result.atsKeywords.technicalSkills, 'Soft skills': result.atsKeywords.softSkills, 'Already in CV': result.atsKeywords.alreadyInCv }).map(([lbl, words]: any) => words?.length ? (
                              <div key={lbl} style={{ marginBottom: 10 }}>
                                <div style={{ fontSize: 13, fontWeight: 500, color: '#888780', marginBottom: 5 }}>{lbl.toUpperCase()}</div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                                  {words.map((w: string) => <span key={w} style={{ fontSize: 13, padding: '2px 8px', borderRadius: 8, background: lbl === 'Already in CV' ? '#E1F5EE' : '#F1EFE8', color: lbl === 'Already in CV' ? '#085041' : '#444441' }}>{w}</span>)}
                                </div>
                              </div>
                            ) : null)}
                          </div>
                          ...
                          <div style={{ paddingTop: 10 }}>
                            {profile?.plan === 'free' ? (
                              <div style={{ textAlign: 'center', padding: '24px 10px' }}>
                                ...
                                <p style={{ fontSize: 15, color: '#5F5E5A', margin: '0 0 14px', lineHeight: 1.6 }}>
                                  Interview preparation — likely questions, key talking points, and questions to ask — is a Pro feature.
                                </p>
                                <button onClick={upgradeNow} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '9px 18px', borderRadius: 7, fontSize: 15, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                              ...
                              <>
                                {result.interviewPrep?.likelyQuestions?.length > 0 && (
                                  <div style={{ marginBottom: 16 }}>
                                    <div style={{ fontSize: 13, fontWeight: 500, color: '#888780', marginBottom: 8 }}>LIKELY QUESTIONS</div>
                                ...
                                {result.interviewPrep?.talkingPoints?.length > 0 && (
                                  <div style={{ marginBottom: 16 }}>
                                    <div style={{ fontSize: 13, fontWeight: 500, color: '#888780', marginBottom: 8 }}>KEY TALKING POINTS</div>
                                ...
                                {result.interviewPrep?.questionsToAsk?.length > 0 && (
                                  <div>
                                    <div style={{ fontSize: 13, fontWeight: 500, color: '#888780', marginBottom: 8 }}>QUESTIONS TO ASK THEM</div>
                ...
                <button onClick={() => { setStep('idle'); setResult(null); setSubmitResult(null) }} style={{ marginTop: 6, background: 'transparent', border: '1px solid #E2E0D8', padding: '10px 20px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Apply to another role</button>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            {applications.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: '#888780' }}>
                ...
                <p style={{ fontSize: 17 }}>No applications yet.</p>
                <button onClick={() => setTab('run')} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: 'pointer', marginTop: 12, fontFamily: 'inherit' }}>Apply now</button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {applications.map(app => (
                  <div key={app.id} style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 16, fontWeight: 500 }}>{app.job_title}</div>
                      <div style={{ fontSize: 14, color: '#888780', marginTop: 2 }}>{app.company} · {app.location} · {new Date(app.submitted_at).toLocaleDateString()}</div>
                      {app.submission_method === 'email' && <div style={{ fontSize: 13, color: '#1D9E75', marginTop: 3 }}>📧 Sent via email</div>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      {app.match_score && <span style={{ fontSize: 15, fontWeight: 500, color: '#1D9E75' }}>{app.match_score}%</span>}
                      <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 10, fontWeight: 500, background: app.status === 'submitted' ? '#E6F1FB' : app.status === 'interview' ? '#E1F5EE' : '#F1EFE8', color: app.status === 'submitted' ? '#0C447C' : app.status === 'interview' ? '#085041' : '#5F5E5A' }}>{app.status}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab === 'account' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 12, padding: 24 }}>
              <h2 style={{ fontSize: 17, fontWeight: 500, margin: '0 0 16px' }}>Account details</h2>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
                <div style={{ background: '#F8F7F4', borderRadius: 8, padding: '12px 14px' }}>
                  <div style={{ fontSize: 13, color: '#888780', marginBottom: 3 }}>Email</div>
                  <div style={{ fontSize: 15, fontWeight: 500 }}>{user?.email}</div>
                </div>
                <div style={{ background: '#F8F7F4', borderRadius: 8, padding: '12px 14px' }}>
                  <div style={{ fontSize: 13, color: '#888780', marginBottom: 3 }}>Plan</div>
                ...
                <div style={{ background: '#F8F7F4', borderRadius: 8, padding: '12px 14px' }}>
                  <div style={{ fontSize: 13, color: '#888780', marginBottom: 3 }}>Applications this month</div>
                  <div style={{ fontSize: 15, fontWeight: 500 }}>{profile?.applications_used_month || 0}{profile?.plan === 'free' ? ' / 5' : profile?.plan === 'africa' ? ' / 30' : ' / 100'}</div>
                </div>
              </div>
              {profile?.plan === 'free' && (
                <button onClick={upgradeNow} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #E2E0D8', borderRadius: 12, padding: 24 }}>
              <h2 style={{ fontSize: 17, fontWeight: 500, margin: '0 0 6px', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Mail size={16} color="#1D9E75"/>Auto-apply email setup
              </h2>
              <p style={{ fontSize: 15, color: '#5F5E5A', margin: '0 0 20px', lineHeight: 1.6 }}>
              ...
              <div style={{ background: '#E1F5EE', border: '1px solid #9FE1CB', borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#085041', marginBottom: 6 }}>
                  How to get a {emailProvider === 'gmail' ? 'Gmail' : emailProvider === 'outlook' ? 'Outlook' : 'Yahoo'} app password:
                </div>
                {emailProvider === 'gmail' && (
                  <ol style={{ margin: 0, paddingLeft: 16, fontSize: 14, color: '#0F6E56', lineHeight: 1.8 }}>
              ...
              {emailStatus && (
                <div style={{ padding: '10px 14px', borderRadius: 8, background: emailStatus.startsWith('✅') ? '#E1F5EE' : emailStatus.startsWith('❌') ? '#fff5f5' : '#F8F7F4', border: `1px solid ${emailStatus.startsWith('✅') ? '#9FE1CB' : emailStatus.startsWith('❌') ? '#F09595' : '#E2E0D8'}`, marginBottom: 14 }}>
                  <span style={{ fontSize: 15 }}>{emailStatus}</span>
                </div>
              )}

              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={saveEmailSettings} disabled={emailSaving} style={{ background: '#1D9E75', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: emailSaving ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
                  {emailSaving ? <Loader size={13} style={{ animation: 'spin 1s linear infinite' }}/> : <CheckCircle size={13}/>}
                  Save email settings
                </button>
                <button onClick={testEmailConnection} disabled={emailTesting} style={{ background: 'transparent', border: '1px solid #E2E0D8', padding: '10px 16px', borderRadius: 8, fontSize: 15, fontWeight: 500, cursor: emailTesting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
